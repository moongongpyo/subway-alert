...
> **2026-09-19 현재 범위 변경:** 한 도구의 체험, 맥락에 맞는 다음 실험, 선택한 실행 한 건의 Markdown 리포트까지만 지원합니다. 아래 초기 기획 중 대안 서비스 추천·재체험·서비스 간 자동 비교·비교 리포트·최종 도구 선택·모델 리포트 호출은 현재 범위에서 제외합니다. 최신 결과 화면 계약과 지원 경계는 [결과 분석 구현 문서](docs/result-analysis.md)가 우선합니다.
