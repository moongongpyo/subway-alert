...
메인 `/`에는 장소 검색과 경로 안내만 둡니다. 좌표는 검색 결과에서 내부적으로 사용합니다. MVP에서는 등록된 목업 장애가 메인 페이지의 모든 경로 검색에 바로 적용됩니다. 별도 시연 모드로 진입할 필요가 없습니다. 예시 경로는 시연 페이지에서만 선택할 수 있습니다. 열린 메인 검색은 장애 시작·종료·해제를 주기적으로 반영하며, 경로 자체가 5분 후 만료되면 다시 검색해야 합니다.
