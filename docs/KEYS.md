...
OpenAI 키는 Railway Spring 서버로 이전했습니다. Daytona 키와 preview token은 현재 경로·관측 기능에서 사용하지 않습니다. 아래 이전 배치 설명은 이 내용으로 대체됩니다. TMAP/서울시 발급 링크는 그대로 유효합니다.
...
- `AGENT_TOKEN`은 `python scripts/prepare_config.py`가 생성합니다. 발급받을 필요가 없습니다. `.deploy/secrets.json`에 저장되며 Git에서 제외됩니다.
...
운영 제어는 사용자 승인에 따라 토큰 없이 공개됩니다. 기존 `APP_ADMIN_TOKEN`은 더 이상 인증에 사용하지 않습니다. 에이전트 간 인증(`AGENT_TOKEN`)은 유지합니다.
