...
## 서울교통공사 getNtceList 추가

공식 공지 수집, 서버 캐시/호출 예산, 경로·대시보드 원문 표시를 추가했습니다. 사용자 서울시 키의 실제 응답 성공을 확인했으며, 종료 시각과 미확인 상태를 구분합니다. 기존 위치·도착 추정과 별개입니다. Java 테스트 20개 통과 및 JavaScript 문법 검사 완료. TMAP 키는 이후 사용자 제공 값으로 설정했습니다.

## TMAP 및 운영 제어 변경

TMAP 사용자 키를 로컬 및 Railway에 등록했습니다. 설정 화면에서 공지 키와 위치·도착 키 상태를 분리했습니다. 사용자 명시 승인에 따라 모드 변경·수집·시연 API와 화면을 토큰 없이 사용하도록 변경했습니다. 변경 후 Java 테스트 20개 및 JS 문법 검사를 통과했습니다.
