...
function renderJob(job){
  ...
  const detail=el('details',{class:'job-details'},el('summary',{text:'실행 내역과 사용량'}),el('div',{class:'usage-grid'},el('div',{},el('small',{text:job.usage.rentalCalls?'토큰 과금 · GPU 임대료 별도':'모델 비용 · 보수적 상한'}),el('strong',{text:`${money(job.usage.micros)} / ${money(job.policy.jobMicros)}`})),el('div',{},el('small',{text:'모델 요청'}),el('strong',{text:`${job.usage.calls} / ${job.policy.calls}${job.usage.openaiCalls?' · OpenAI '+job.usage.openaiCalls+'회':''}`})),el('div',{},el('small',{text:'전체 수정'}),el('strong',{text:`${job.round} / ${job.policy.repairs}`})),el('div',{},el('small',{text:'실행 환경'}),el('strong',{text:job.executionMode==='node-api'?'Node.js 직접 호출':job.sandboxId?job.sandboxId.slice(0,8):'준비 전'}))),el('p',{class:'data-note',text:`입력 ${job.usage.input.toLocaleString()} · 출력 ${job.usage.output.toLocaleString()} 토큰 · 예약/미확정 ${money(job.usage.reserved)}. ${job.usage.rentalCalls?'Nosana GPU 시간 요금은 별도이며 0달러가 아닙니다. ':''}${job.executionMode==='node-api'?'샌드박스 사용 없음 · 외부 API 비용 별도.':'Daytona와 외부 API 비용 별도.'}`}),el('div',{class:'evidence-list'},...job.evidence.map(e=>el('p',{text:`${e.role} · ${e.description||e.scenario||'검증 완료'} · ${e.version||''}`}))),el('pre',{class:'log-output',text:[...(job.failures||[]).map(f=>({at:f.at,text:f.code+' · '+f.message})),...job.logs].map(l=>`[${new Date(l.at).toLocaleTimeString('ko-KR')}] ${l.text}`).join('\n')||'아직 실행 로그가 없습니다.'}));
...
try{
  ...
  if(config.infrastructure&&!config.infrastructure.available){error(`Daytona 예약 ${config.infrastructure.used}/${config.infrastructure.limit}분 · 오픈소스용 새 환경을 만들 수 없습니다. API 직접 호출은 이용 가능합니다. 초기화: ${new Date(config.infrastructure.resetsAt).toLocaleString()}`);}
  $('#connection-status').replaceChildren(...[[c.model?.label||'OpenAI API',c.model?.ready??c.openai],...(c.model?.provider==='nosana'?[['OpenAI 자동 복구',c.model.fallback?.ready]]:[]),['Daytona · 오픈소스 실행용',c.daytona],['실행 스냅샷',c.snapshot]].map(([name,value])=>el('div',{class:'connection-row'},el('strong',{text:name}),el('span',{class:'pill '+(value?'green':''),text:value?'설정됨':name==='실행 스냅샷'?'기본 이미지 사용':'설정 필요'}))));
  if(!(c.model?.ready??c.openai)){$('#config-notice').hidden=false;$('#config-notice').append(el('span',{text:`연결 준비 · ${c.model?.label||'모델 연결'} 키를 설정하면 실제 체험을 시작할 수 있어요.`}),el('button',{class:'text-button',text:'설정 보기 ↗',onclick:()=>$('#settings').showModal()}));}
