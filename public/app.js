...
try{
  ...
  $('#connection-status').replaceChildren(...[[c.model?.label||'OpenAI API',c.model?.ready??c.openai],['Daytona · 오픈소스 실행용',c.daytona],['실행 스냅샷',c.snapshot]].map(([name,value])=>el('div',{class:'connection-row'},el('strong',{text:name}),el('span',{class:'pill '+(value?'green':''),text:value?'설정됨':name==='실행 스냅샷'?'기본 이미지 사용':'설정 필요'}))));
