...
try{
  ...
  $('.settings-note').textContent=['로컬 단일 사용자 모드 · 자동 수정 최대 '+config.limits.repairs+'회','모델 비용: 작업당 $'+config.limits.jobUSD+(config.limits.userDayUSD==null?'':' · 하루 $'+config.limits.userDayUSD),'외부 API 및 Daytona 비용은 별도 한도로 관리합니다.'].join('\n');
  ...
  $('#connection-status').replaceChildren(...[['OpenAI API',c.openai],['Daytona',c.daytona],['실행 스냅샷',c.snapshot]].map(([name,value])=>el('div',{class:'connection-row'},el('strong',{text:name}),el('span',{class:'pill '+(value?'green':''),text:value?'설정됨':name==='실행 스냅샷'?'기본 이미지 사용':'설정 필요'}))));
  if(!c.openai||!c.daytona){$('#config-notice').hidden=false;$('#config-notice').append(el('span',{text:`연결 준비 · ${[!c.openai?'OpenAI API':'',!c.daytona?'Daytona':''].filter(Boolean).join(', ')} 키를 설정하면 실제 체험을 시작할 수 있어요.`}),el('button',{class:'text-button',text:'설정 보기 ↗',onclick:()=>$('#settings').showModal()}));}
