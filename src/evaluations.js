...
export class Evaluations {
  ...
  async run(eid,owner,experimentId,key,{manualOutput,manualConditions,manualState}={}){
    const prepared=this.store.tx(()=>{
      ...
      return idempotent(this.db,owner,key,digest({eid,experimentId,manualOutput,manualConditions,manualState}),()=>{
        ...
        const run=putRecord(this.db,eid,'run',{experimentId,jobId:j.id,goalVersion:ex.goalVersion,version:ex.version,fields:j.plan.fields,viewer:j.viewer,input:ex.input,title:ex.title,reason:ex.reason,changes:ex.changes,check:ex.check,parentRunId:ex.basedOn.runId,createdAt:Date.now(),state:'running',provenance:j.plan.hasUI?'manual':j.executionMode==='node-api'?'node-api':'gateway',manualConditions:String(manualConditions||'').slice(0,3000),environment:{runtime:j.executionMode==='node-api'?'Node.js '+j.nodeVersion:j.plan.runtime,database:j.plan.database.kind,commit:j.source?.commit||null,resources:j.executionMode==='node-api'?'앱 서버 직접 HTTP 호출 · Daytona 미사용':'Daytona 정책 한도: 2 CPU / 4 GiB / 20 GiB',databaseState:j.executionMode==='node-api'?'로컬 DB 없음; 원격 API 상태는 제공사 관리':'초기화되지 않은 동일 환경; 이전 실행 상태가 남을 수 있음'},cost:{externalMicros:j.plan.hasUI||j.requestMicros===null?null:j.requestMicros||0,kind:j.plan.hasUI||j.requestMicros===null?'unknown':j.apiAccess?.pricingSource==='user_free_allowance'?'user_declared_free':'reserved_ceiling',daytona:j.executionMode==='node-api'?'not_used':'unknown'}});
    ...
    try{
      ...
      run={...run,state,status:result.status,output,storageError,deterministicChecks,durationMs:j.plan.hasUI?null:Math.round(performance.now()-started),measurement:j.plan.hasUI?'사용자 등록; 실행 시간 미측정':j.executionMode==='node-api'?'앱 Node.js 서버→외부 API 왕복 1회; 준비 시간 제외':'제어 서버→샌드박스 왕복 1회; 준비 시간 제외',finishedAt:Date.now()};
  ...
  task(eid,owner,type,fingerprint,payload,perform){
    ...
    const recoveryPolicy=this.models.recoveryPolicy?.(policy,{image:asset})||policy;
    const job=this.store.create(owner,this.store.get(payload.basedOn?.jobId)?.url||'https://example.com',`${type}-${eid}-${fingerprint.slice(0,25)}`,recoveryPolicy,{evaluationId:eid,taskType:type});
