...
export function hostingConfig(env=process.env) {
  const host=env.HOST||'127.0.0.1', remote=!['127.0.0.1','localhost','::1'].includes(host),publicDemo=env.PUBLIC_DEMO==='true';
  let origin;
  if(remote){
    ...
    if(!publicDemo&&(!env.ACCESS_PASSWORD||env.ACCESS_PASSWORD.length<24))throw new Error('비공개 배포에는 24자 이상의 ACCESS_PASSWORD가 필요합니다.');
  }
  return {host,remote,publicDemo,origin:origin?.origin,hostname:origin?.hostname,password:env.ACCESS_PASSWORD,username:env.ACCESS_USERNAME||'admin'};
}
...
export function hostingGate(config) {
  ...
  return (req,res,next)=>{
    ...
    if(!config.publicDemo&&!timingSafeEqual(hash(supplied),hash(`${config.username}:${config.password}`)))return res.status(401).set('WWW-Authenticate','Basic realm="URL to Playground", charset="UTF-8"').send('로그인이 필요합니다.');
