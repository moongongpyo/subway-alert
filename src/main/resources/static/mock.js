...
const from=new PlacePicker($('#mock-from'),'출발'),to=new PlacePicker($('#mock-to'),'도착');let route,legs=[],loading=false,savedPlaces,savedProvider,savedDeparture,immediateStart=true;
const departurePicker=new DeparturePicker($('#departure-picker'));
...
$('#mock-search').onsubmit=async e=>{e.preventDefault();if(loading)return;loading=true;$('#mock-find').disabled=true;$('#event-form').hidden=true;try{const example=$('#mock-example').checked;const a=example?{name:'강남',lon:127.027619,lat:37.497952}:from.get(),b=example?{name:'선릉',lon:127.048203,lat:37.504286}:to.get();const searchDttm=departurePicker.get();route=await api('/routes','POST',{from:a,to:b,provider:example?'DEMO':'TMAP',searchDttm});savedDeparture=searchDttm;savedPlaces={from:a,to:b};savedProvider=example?'DEMO':'TMAP';$('#event-status').textContent='';legs=[];const seen=new Set();for(const j of route.plan.journeys)for(const l of j.legs){const identity=l.mode+l.route+l.start+l.end;if(['BUS','SUBWAY'].includes(l.mode)&&l.stops.length>=2&&!seen.has(identity)){legs.push(l);seen.add(identity);}}$('#mock-leg').replaceChildren();for(const [i,l] of legs.entries()){const o=el('option',l.route+' · '+l.start+' → '+l.end);o.value=i;$('#mock-leg').append(o);}$('#event-form').hidden=!legs.length;$('#mock-status').textContent=legs.length?'장애를 적용할 승차 구간과 정류장을 선택하세요.':'정류장 정보가 있는 경로가 없습니다.';selectLeg();}catch(err){$('#mock-status').textContent=err.message;}finally{loading=false;$('#mock-find').disabled=false;}};
...
async function refreshSelection(selected){
 ...
 const fresh=await api('/routes','POST',{...savedPlaces,provider:savedProvider,searchDttm:savedDeparture});
