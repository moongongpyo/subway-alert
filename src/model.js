...
export class Models {
  constructor(store, key=process.env.OPENAI_API_KEY) { this.store=store; this.client=key?new OpenAI({apiKey:key,maxRetries:0,timeout:90_000}):null; }
  ...
  async ask(id,role,task,context,schema,{small=false,escalate=false,signal,images=[]}={}) {
    if(!this.client) fail('CONFIG_REQUIRED','OPENAI_API_KEY 설정이 필요합니다.',503);
    if(day()>PRICE_REVIEW_UNTIL) fail('PRICE_REVIEW_REQUIRED','모델 요율 검토 유효기간이 지났습니다. 운영자 확인이 필요합니다.');
    ...
    // Count the exact request, including instructions and schema. No hidden conversation state.
    const count=await this.client.responses.inputTokens.count(body,{signal,timeout:Math.max(1,Math.min(20_000,this.store.remaining(this.store.get(id))))});
    ...
    for(let attempt=0;attempt<2;attempt++) {
      ...
      try {
        const r=await this.client.responses.create({...body,store:false,max_output_tokens:j.policy.outputPerCall,service_tier:'default'},{signal,timeout:Math.max(1,Math.min(j.policy.modelMs,this.store.remaining(j)))});
