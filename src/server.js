...
export function createApp({store=new Store(),models,sandboxes,orchestrator,hosting=hostingConfig(),evaluationOptions={}}={}) {
  models??=new Models(store);sandboxes??=new Sandboxes(store);orchestrator??=new Orchestrator(store,models,sandboxes);
  ...
  const publicJob=j=>{
    ...
    for(const key of ['owner','controlToken','previewToken','databaseUrl','source','sessions','root','sandboxName','lastFailure','journal','recipeFiles'])delete copy[key];
  ...
  app.get('/api/evaluations/:eid/reports/:reportId/download',(req,res)=>{evaluations.get(req.params.eid,req.owner);const r=evaluations.record(req.params.eid,req.params.reportId,'report');if(r.expiresAt<=Date.now())fail('EXPIRED','리포트 파일이 만료됐습니다.',410);if(!r.markdown)fail('NOT_READY',r.error||'리포트를 준비 중입니다.',409);res.set('Content-Disposition',`attachment; filename="execution-${r.id}.md"`).type('text/markdown').send(r.markdown);});
