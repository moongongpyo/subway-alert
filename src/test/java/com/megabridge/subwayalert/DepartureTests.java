...
class DepartureTests {
 ...
 @Test void forwardsDepartureAndSeparatesCache()throws Exception{
  ...
  try{
   var store=mock(Store.class);when(store.reserveProviderCalls(eq("TMAP"),eq(1),anyInt())).thenReturn(true);
   var json=JsonMapper.builder().build();var client=new TmapClient(store,json);
   ReflectionTestUtils.setField(client,"key","test");ReflectionTestUtils.setField(client,"endpoint","http://127.0.0.1:"+server.getAddress().getPort()+"/routes");ReflectionTestUtils.setField(client,"budget",10);
