...
class PlaceAndMockTests {
 ...
 @Test void timeBoundariesAndMainRoutesApplySharedMocks() {
  ...
  try {
   var normal=service.create("owner",PLACES.get("강남"),PLACES.get("선릉"),"DEMO",false);assertEquals("route-2",normal.recommendedId());assertEquals("MOCK",normal.blocks().getFirst().source());
   ...
   when(store.list("mock-disruption","demo",100,MockDisruptions.Event.class)).thenReturn(List.of());assertEquals("route-1",service.get("owner",simulation.id()).recommendedId());assertEquals("route-1",service.get("owner",normal.id()).recommendedId());
   when(store.list("mock-disruption","demo",100,MockDisruptions.Event.class)).thenReturn(List.of(event));assertEquals("route-2",service.get("owner",simulation.id()).recommendedId());assertEquals("route-2",service.get("owner",normal.id()).recommendedId());
