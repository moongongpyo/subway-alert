...
class TmapClientTests {
    ...
    @Test void sendsOfficialContractCachesAndPreservesStops() throws Exception {
        var store=mock(Store.class); when(store.reserveProviderCalls("TMAP",1,10)).thenReturn(true);
        ...
        try {
            ...
            ReflectionTestUtils.setField(client,"key","test-key"); ReflectionTestUtils.setField(client,"budget",10);
            ...
            verify(store,times(1)).reserveProviderCalls("TMAP",1,10);
            when(store.reserveProviderCalls("TMAP",1,10)).thenReturn(false);
            assertEquals(429,assertThrows(ResponseStatusException.class,()->client.routes(PLACES.get("역삼"),PLACES.get("선릉"))).getStatusCode().value());
