...
const result={summary:'다음 조건 확인',questions:[],assessments:[],html:'<main><h2>검색 결과</h2><strong data-stat="count" data-source="/items"></strong><article data-each="/items"><h3 data-value="./name"></h3><span data-value="./stars" data-format="number"></span></article></main>',css:'article{padding:16px}',cards:['문서 OCR','표 추출','한글 인식'].map((title,i)=>({title,kind:'coverage',reason:'검색 범위를 비교',check:'관련 저장소가 반환되는지',changes:[{field:'q',valueJson:JSON.stringify(['document ocr','table extraction','korean ocr'][i]),evidence:'Search query'}],requiresInput:[],asset:null}))};
